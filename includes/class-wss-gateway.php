<?php
/**
 * Woo Stripe Subscription Payment Gateway
 *
...
/**
 * Wss Payment Gateway Class
 *
 * @since    1.0.0
 * @class    Wss_Payment_Gateway
 * @extends  WC_Payment_Gateway_CC
 */
class Wss_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	/**
	 * True if Wss should save tokenized payment methods for future use.
	 *
	 * @since  1.0.0
	 * @var    boolean
	 */
	...
	/**
	 * True if Wss should support Bitcoin transactions.
	 *
	 * @since  1.0.0
	 * @var    boolean
	 */
	...
	/**
	 * True if Wss should support Alipay transactions.
	 *
	 * @since  1.0.0
	 * @var    boolean
	 */
	...
	public function __construct() {
		...
		$this->id = WSS_PLUGIN_ID;
		$this->method_description = WSS_PLUGIN_DESC . '<hr>';
		$this->method_title = WSS_PLUGIN_TITLE;
		...
		$this->title = $this->get_option( 'title', WSS_PLUGIN_TITLE );
		$this->description = $this->get_option( 'description', NULL );
		$this->order_button_text = $this->get_option( 'order_button_text', esc_html__( 'Place Order', 'woo-stripe-subscription' ) );
		...
		$this->stripe_checkout_button = $this->get_option( 'stripe_checkout_button', esc_html__( 'Pay {{amount}}', 'woo-stripe-subscription' ) );
		...
		$this->key = Wss::get_api_key( 'secret' );
		$this->currency = strtoupper( get_woocommerce_currency() );
		add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
		add_action( 'admin_notices', array( $this, 'wss_gateway_admin_notices' ) );
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields = include( 'settings-wss.php' );
	}
	...
	public function process_payment( $order_id ) {
		...
		if( ! isset( $_POST['wss-data'] ) ) {
			...
			if( ! isset( $customer->stripe ) ) {
				...
				$customer->_object = Wss_API::request( 'customers', $this->key, $customer->params );
			...
			if( $data['method'] === 'saved' ) {
				...
				$customer->source = Wss_API::request(
			...
			$params = array(
				"amount" => $this->wss_get_zero_decimal( $order->get_total() ),
			...
			$charge = Wss_API::request( 'charges', $this->key, $params );
			...
			$refund = Wss_API::request( 'refunds', $this->key, $params );
			...
			foreach( $cart as $cart_item_key => $cart_item ) {
				$item = $cart_item['data'];
				if( $item->get_type() === 'wss_subscription' ) {
			...
			//	throw an exception.
			$amount = $this->wss_get_zero_decimal( $to_pay['total'] - $to_pay['subscriptions'] );
			if( $amount > 0 ) {
				if( ! $this->wss_minimum_order( $amount ) ) {
			...
			foreach( $cart as $cart_item_key => $cart_item ) {
				...
				if( $item->get_type() === 'wss_subscription' ) {
					...
					$subscription = Wss_API::request( 'subscriptions', $this->key, $params );
					...
					$order->add_order_note( sprintf(
						esc_html__( 'Subscription authorized and captured: %s', 'woo-stripe-subscription' ),
						$view_sub_url
					) );
			...
			if( $amount > 0 ) {
				$params = array(
					...
					"customer" => $customer->stripe,
					"source" => $source,
					"description" => get_post_meta( $order->get_id(), '_wss_order_description', true ),
				...
				$charge = Wss_API::request( 'charges', $this->key, $params );
				if( isset( $charge ) ) {
					$this->wss_set_order_charge_id( $order_id, $charge->id );
					...
					if( $this->payment_action === 'capture' ) {
						update_post_meta(
							$order_id,
							'_wss_order_funds_captured',
							1
						);
						$order->add_order_note( sprintf(
							esc_html__( 'Payment authorized and captured: %s', 'woo-stripe-subscription' ),
							$view_charge_url
						) );
						...
						update_post_meta(
							$order_id,
							'_wss_order_uncaptured_charge',
							$charge->id
						);
						$order->add_order_note( sprintf(
							esc_html__( 'Payment authorized: %s', 'woo-stripe-subscription' ),
							$view_charge_url
						) );
					}
	...
	public function process_refund( $order_id, $amount = null, $reason = null ) {
		...
		$charge_id = get_post_meta( $order_id, '_wss_order_charge_id', true );
		$charge = Wss_Api::request( "charges/{$charge_id}", $this->key );
		if( isset( $charge  ) ) {
			if( $charge->source->object === 'bitcoin_receiver' ) {
				...
				$params = array(
					'charge'    => $charge->id,
					'amount'    => $this->wss_get_zero_decimal( $amount ),
					'metadata'  => array(
				...
				if( isset( $refund  ) ) {
					$order = wc_get_order( $order_id );
					$order->add_order_note( sprintf(
						esc_html__( 'Refunded %s%s%s%s: %s', 'woo-stripe-subscription' ),
						'<strong>',
						(string) get_woocommerce_currency_symbol(),
						(string) $this->wss_format_currency_unit( $refund->amount, $refund->currency ),
						'</strong>',
						sprintf(
	 ...
	 * @param  int
	 */
	private function wss_set_order_funds_captured( $order_id ) {

		update_post_meta( $order_id, '_wss_order_funds_captured', true );
	}
	 ...
	 * @param  int
	 */
	private function wss_set_order_charge_id( $order_id, $charge_id ) {

		update_post_meta( $order_id, '_wss_order_charge_id', $charge_id );
	}
	...
	public function add_payment_method() {
		...
		$customer = Wss_Customer::load( get_current_user_id() );
		if( isset( $customer ) ) {
			if( !isset( $_POST['wss-data'] ) ) {
				//	data is missing or null.
				wc_add_notice( esc_html__( 'There was an error processing your request.', 'woo-stripe-subscription' ), 'error' );
				return array(
					'result'   => 'failure'
				);
			} else {
				$data = json_decode( wp_filter_post_kses( $_POST['wss-data'] ), true );
				if( isset( $data['error'] ) ) {
	 ...
	 * @hook   admin_notices
	 */
	public function wss_gateway_admin_notices() {

		/*
		$screen = get_current_screen();
		$section = isset( $_GET['section'] ) && $_GET['section'] === WSS_PLUGIN_ID ? true : false ;
		if( $section && $screen->id === 'woocommerce_page_wc-settings' ) {
			echo '<div class="notice notice-warning"><p>';
			echo esc_html__( 'Put admin notices here...', 'woo-stripe-subscription' );
			echo '</p></div>';
		}
		*/
	}
	...
	public function form() {
		...
		if( $this->stripe_checkout_enabled && !is_add_payment_method_page() ) {
			...
			include( WSS_PLUGIN_DIR_PATH.'templates/checkout/wss-stripe-checkout.php' );
		} else {
			//	inline credit card form.
			include( WSS_PLUGIN_DIR_PATH.'templates/checkout/wss-inline-cc-form.php' );
		}
	}
	...
	public function payment_fields() {
		...
		if( null !== Wss::get_api_key( 'publishable' ) ) {
			if( is_checkout() ) {
	 ...
	 * @return  int
	 */
	public function wss_get_zero_decimal( $total = null ) {

		return Wss::get_zero_decimal( $total, $this->currency );
	}
	 ...
	 * @param   mixed
	 */
	private function wss_format_currency_unit( $amount, $currency ) {

		return Wss::format_currency_unit( $amount, $currency );
	}
	 ...
	 * @return  boolean
	 */
	private function wss_minimum_order( $amount ) {
	 ...
	 * @return  string
	 */
	public function wss_get_store_name() {
	 ...
	 * @return  boolean
	 */
	public function wss_supports( $capability = null ) {
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_order_status_processing( $order_id ) {

		if( ! get_post_meta( $order_id, '_wss_order_funds_captured', true ) ) {
			$uncaptured = get_post_meta( $order_id, '_wss_order_uncaptured_charge', true );
			if( isset( $uncaptured ) && ! empty( $uncaptured ) ) {
				$charge = Wss_API::request(
				...
				if( isset( $charge ) ) {
					...
					$order->add_order_note( sprintf(
						esc_html__( 'Payment captured: %s', 'woo-stripe-subscription' ),
						$view_charge_url
					) );
					$order->payment_complete( $charge->id );
				}
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_order_status_completed( $order_id ) {
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_order_status_cancelled( $order_id ) {
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_order_status_refunded( $order_id ) {
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_coupon_get_discount_amount( $discount, $discounting_amount, $cart_item, $single ) {

		if( $cart_item['data']->get_type === 'wss_subscription' ) {
			$discount = 0;
		}
		return $discount;
	}
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_payment_token_set_default( $token_id ) {

		$customer = Wss_Customer::load( get_current_user_id() );
		$token = WC_Payment_Tokens::get( $token_id );
		if( WSS_PLUGIN_ID === $token->get_gateway_id() ) {
			$params = array(
				"default_source" => $token->get_token(),
			);
			$update = Wss_API::request( "customers/{$customer->stripe}", $this->key, $params );
	 ...
	 * @since  1.0.0
	 */
	public function wss_payment_gateway_woocommerce_payment_token_deleted( $card_id ) {
	...
	public function get_icon() {
		...
		$icon_array = array(
			...
			'bitcoin' => array(
				'url' => WSS_PLUGIN_DIR_URL.'/assets/images/icons/bitcoin'.$ext,
				'alt' => 'Bitcoin',
				'width' => 16,
