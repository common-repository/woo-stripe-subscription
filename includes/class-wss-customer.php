<?php
/**
 * Woo Stripe Subscription Customer
 *
...
class Wss_Customer {
	 ...
	 * method Wss_Customer::load(), the object can return
	...
	private function get_stripe() {
		...
		$meta = get_user_meta( $this->uid, WSS_PLUGIN_MODE.'_stripe_id', true );
	...
	public function save() {
		...
		return update_user_meta( $this->uid, WSS_PLUGIN_MODE.'_stripe_id', $this->stripe );
	}
	...
	public function roles( $item ) {
		...
		if( ! $this->guest ) {
			if( isset( $item->product_custom_fields['_wss_subscription_product_user_roles'][0] ) ) {
				return array(
					'roles' => implode(
						',',
						maybe_unserialize(
							$item->product_custom_fields['_wss_subscription_product_user_roles'][0]
						)
					)
				);
			}
	...
	public function has_subscribed_to( $subscription ) {
		...
		if( ! $this->guest ) {
			...
			$user_subs = maybe_unserialize( get_user_meta( $this->uid, WSS_PLUGIN_MODE.'_subscriptions', true ) );
			if( isset( $user_subs ) && is_array( $user_subs ) ) {
				update_user_meta( $this->uid, WSS_PLUGIN_MODE.'_subscriptions', array_unique( array_merge( $user_subs, $plan_id ) ) );
			} else {
				update_user_meta( $this->uid, WSS_PLUGIN_MODE.'_subscriptions', $plan_id );
			}
			if( isset( $subscription->metadata->roles ) ) {
				$user = new WP_User( $this->uid );
				$roles = explode( ',', $subscription->metadata->roles );
	...
	public function save_source() {
		...
		$token->add_meta_data( 'mode', WSS_PLUGIN_MODE, true);
		$token->add_meta_data( 'customer', $this->stripe, true);
		$token->set_token( $this->source->id );
		$token->set_gateway_id( WSS_PLUGIN_ID );
	...
	public function get_saved_source( $id = null ) {
		...
		if( (int) $token->get_user_id() !== (int) $this->uid || $token->get_meta( 'mode') !== WSS_PLUGIN_MODE ) {
			return null;
		} else {
